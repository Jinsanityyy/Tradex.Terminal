...
export function CinematicClientLayer() {
  ...
  useEffect(() => {
    // All browser-specific imports are lazy — no top-level GSAP/Lenis in this module
    (async () => {
      const { gsap } = await import("gsap");
      const { ScrollTrigger } = await import("gsap/ScrollTrigger");
      gsap.registerPlugin(ScrollTrigger);

      // Pre-hide hero elements (loader covers them while they wait)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // ── Cards (features + pricing) — stagger by column
