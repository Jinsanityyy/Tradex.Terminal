...
import { MessageCircle, X, Send, Loader2, Hash, Users } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("group_chat", { config: { broadcast: { self: false } } })
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => {
        ...
        if (msg.recipient_id) return; // skip DMs entirely
        if (msg.user_id === userId) return; // already added optimistically
      ...
      {open && (
        <div className="fixed bottom-20 right-5 z-50 w-[340px] flex flex-col rounded-2xl border border-[hsl(var(--border))] bg-[hsl(220,18%,6%)] shadow-2xl overflow-hidden" style={{ maxHeight: "520px" }}>
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-b border-[hsl(var(--border))] bg-[hsl(220,18%,8%)] shrink-0">
            ...
            <span className="text-sm font-semibold text-white flex-1">Group Chat</span>
            <div className="flex items-center gap-1 mr-2">
              <Users className="h-3 w-3 text-[hsl(var(--muted-foreground))]" />
              <span className="text-[10px] text-[hsl(var(--muted-foreground))]">{onlineCount}</span>
            </div>
