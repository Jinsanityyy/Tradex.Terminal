...
export const config = {
  ...
  // /public, which phones would otherwise be redirected away from to /m.
  matcher: ["/((?!_next/static|_next/image|favicon|.*\\.png|.*\\.svg|.*\\.ico|.*\\.json|.*\\.js\\.map|.*\\.mq5|.*\\.html|sw\\.js|workbox-.*).*)"],
};
