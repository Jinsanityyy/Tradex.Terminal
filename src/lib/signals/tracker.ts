...
function resolveFromOHLC(
  ...
  for (const bar of candles) {
    if (direction === "short") {
      // TP checked before SL — if candle low reached TP, the short TP order
      // would have been filled first. SL only wins if TP was not touched.
      if (tp2 !== null && bar.l <= tp2) return { status: "win_tp2", outcome: mkOutcome(entry, tp2, parseFloat((Math.abs(tp2 - entry) / riskDist).toFixed(2)), bar.l) };
      if (bar.l <= tp1)                 return { status: "win_tp1", outcome: mkOutcome(entry, tp1, parseFloat((Math.abs(tp1 - entry) / riskDist).toFixed(2)), bar.l) };
      if (bar.h >= stopLoss)            return { status: "loss_sl", outcome: mkOutcome(entry, stopLoss, -1, bar.h) };
    } else {
      // Long: TP is above entry, SL is below
      if (tp2 !== null && bar.h >= tp2) return { status: "win_tp2", outcome: mkOutcome(entry, tp2, parseFloat((Math.abs(tp2 - entry) / riskDist).toFixed(2)), bar.h) };
      if (bar.h >= tp1)                 return { status: "win_tp1", outcome: mkOutcome(entry, tp1, parseFloat((Math.abs(tp1 - entry) / riskDist).toFixed(2)), bar.h) };
      if (bar.l <= stopLoss)            return { status: "loss_sl", outcome: mkOutcome(entry, stopLoss, -1, bar.l) };
    }
  }
  return null;
}
